...
type Job struct {
	...
	Command string `json:"command"` // 脚本指令
...
type JobExecuteInfo struct {
	...
	CancelContext context.Context // 取消任务command的context
...
type Response struct {
	...
	Errno int `json:"errno"` // 错误码，零代表返回正常
	Msg string `json:"msg"`
	Data interface{} `json:"data"` // 任意类型接口数据
}
...
type JobLog struct {
	...
	Command string `json:"command" bson:"command"` // 任务名字
...
type SortLogByStartTime struct {
	...
	SortOrder int `bson:"startTime"` // 按startTime为-1
}
...
// 从etcd的/cron/killer/jobs10/提取任务名
func ExtractKillerName(killerKey string)(string){
	return strings.TrimPrefix(killerKey,JOB_KILLER_DIR)
}


//任务变化的事件有两种，1:更新任务)2：删除任务)
